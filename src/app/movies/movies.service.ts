...
export class MoviesService {
  ...
  createLocal(movie: Movie) {
    ...
    return this.http
      .post(`${environment.backEnd}/movies`, newMovie)
      .mergeMap(() =>
        this.http
          .post(`${environment.backEnd}/movies-actors`, {
            id: newMovie.id,
            actors
          })
  ...
  createFirebase(movie: Movie) {
    ...
    const newActors = movie.actors.reduce(
      (actors, actor) => ({ ...actors, [actor.id]: true }),
      {}
    );

    return this.http
      .post(`${environment.backEnd}/movies.json`, JSON.stringify(newMovie))
      .map(result => result.json())
      .map(result => ({ ...movie, id: result.name, actors: movie.actors }))
      .mergeMap(result =>
        this.http
          .patch(
            `${environment.backEnd}/movies-actors/${result.id}.json`,
            JSON.stringify(newActors)
          )
          .map(() => result)
      );
  }
  ...
  retrieveLocal(): Observable<Movie[]> {
    return (
      this.http
        .get(`${environment.backEnd}/movies`)
        // .delay(1000)
        .map(movies => movies.json())
        .concatAll()
        .mergeMap(movie =>
          this.http
            .get(`${environment.backEnd}/movies-actors/${movie.id}`)
            .map(movieActors => movieActors.json().actors)
            .concatAll()
            .mergeMap(actorId =>
              this.http
                .get(`${environment.backEnd}/actors/${actorId}`)
  ...
  retrieveFirebase(): Observable<Movie[]> {
    return (
      this.http
        .get(`${environment.backEnd}/movies.json`)
        // .delay(1000)
        .map(movies => movies.json())
        .flatMap(movies => {
          const moviesArray = [];
        ...
        .mergeMap(movie =>
          this.http
            .get(`${environment.backEnd}/movies-actors/${movie.id}.json`)
            ...
            .mergeMap(actorId =>
              this.http
                .get(`${environment.backEnd}/actors/${actorId}.json`)
                .map(actor => actor.json())
                .map(
                  actor =>
                    <Actor>{
                      ...
                      fullname: this.getFullname(actor)
                    }
                )
            )
            .reduce((actors, actor) => [...actors, actor], [])
  ...
  updateLocal(movie: Movie) {
    ...
    return this.http
      .put(`${environment.backEnd}/movies/${movie.id}`, {
        title: movie.title,
        genres: movie.genres,
        year: movie.year,
        director: movie.director
      ...
      .mergeMap(() =>
        this.http.put(`${environment.backEnd}/movies-actors/${movie.id}`, {
          actors: newActors
        })
      );
  }
  ...
  updateFirebase(movie: Movie) {
    ...
    return this.http
      .patch(
        `${environment.backEnd}/movies/${movie.id}.json`,
        JSON.stringify({
          title: movie.title,
          genres: movie.genres,
          year: movie.year,
      ...
      .mergeMap(() =>
        this.http.put(
          `${environment.backEnd}/movies-actors/${movie.id}.json`,
          JSON.stringify(newActors)
        )
      );
  }
  ...
  deleteLocal(id: string) {
    return this.http
      .delete(`${environment.backEnd}/movies/${id}`)
      .mergeMap(() =>
        this.http.delete(`${environment.backEnd}/movies-actors/${id}`)
      );
  }

  deleteFirebase(id: string) {
    return this.http
      .delete(`${environment.backEnd}/movies/${id}.json`)
      .mergeMap(() =>
        this.http.delete(`${environment.backEnd}/movies-actors/${id}.json`)
      );
  }
